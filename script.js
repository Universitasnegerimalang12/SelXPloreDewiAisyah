...
// ==================== ENSURE SPLASH BUTTON WORKS ====================
document.addEventListener("DOMContentLoaded", function () {
  const btnMulai = document.getElementById("btnMulai");
  
  if (btnMulai) {
    console.log("✓ Button Mulai ditemukan");
    ...
    // Keyboard handler
    btnMulai.addEventListener("keydown", function (e) {
      if (e.key === "Enter" || e.key === " ") {
        console.log("✓ Tombol diaktivasi keyboard: " + e.key);
        e.preventDefault();
        e.stopPropagation();
        closeSplashScreen();
      }
    ...
    console.error("✗ Button #btnMulai TIDAK DITEMUKAN!");
  }
});
...
// Close splash screen (used by the splash "Mulai Belajar" button)
function closeSplashScreen() {
  console.log(">>> closeSplashScreen() dipanggil");
  const splashScreen = document.getElementById("splashScreen");
  const mainContent = document.getElementById("mainContent");

  if (splashScreen) {
    console.log(">>> Menghide splash screen...");
    splashScreen.style.display = "none";
    splashScreen.classList.add("hidden");
  }
  
  if (mainContent) {
    console.log(">>> Menampilkan main content...");
    mainContent.style.display = "block";
  }
  
  // remember in session so splash doesn't reappear
  try {
    sessionStorage.setItem("splashClosed", "true");
    console.log(">>> Session storage diupdate");
  } catch (e) {
    // ignore storage errors
  }
  
  console.log(">>> Splash screen ditutup!");
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Close menu when clicking a modal link
  ...
  document.addEventListener("click", function (event) {
    ...
    if (navbar && navMenu && menuToggle) {
      if (!navbar.contains(event.target)) {
        ...
        const openDropdowns = navMenu.querySelectorAll(".dropdown-menu.show");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Ensure splash button has an explicit click handler (fallback for some browsers/extensions)
  const btnMulai = document.getElementById('btnMulai');
  if (btnMulai) {
    // make sure it's clickable
    btnMulai.style.pointerEvents = 'auto';
    btnMulai.style.zIndex = 99999;
    btnMulai.setAttribute('role', 'button');
    btnMulai.setAttribute('tabindex', '0');
    btnMulai.addEventListener('click', function (e) {
      e.stopPropagation();
      closeSplashScreen();
    });
